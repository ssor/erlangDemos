%rfidClient.erl
-module(rfidClient2).
-export([start_server/1,add_ports/1,stop_all_ports/0, start_send_data/1,stop_send_data/1]).
...
start_send_data(Port) ->
	...
	% send(List,Port,List).
	% register(rfid_client,spawn(fun() -> send(List,Port,List) end)).
	%将数据读取到内存中
